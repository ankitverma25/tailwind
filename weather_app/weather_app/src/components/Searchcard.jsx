import React from 'react'
import { Thermometer, Droplets, Cloud, Wind, MapPinned, CircleX } from 'lucide-react'
import useWeather from '../context/Context'
import Searchforecast from './Searchforecast';

function Searchcard() {
    const { hide, sethide, searchdata } = useWeather();

    const change = () => {
        sethide(!hide)
    }

    if (searchdata === null) {
        return <div>No results found or invalid city entered</div>;
    }

    if (!searchdata || searchdata.length === 0) {
        return <div>No results found</div>;
    }

    return (
        <>
            <div className={`${hide ? "hidden" : ""} relative animate-zoom-in bg-white rounded-xl shadow-lg p-6 w-full max-w-md m-4 bg-no-repeat bg-cover bg-[url('data:image/jpeg;base64,')]`}>
                <div className="absolute right-3 top-3"><button onClick={change}><CircleX /></button></div>
                <div className="flex items-center justify-between mb-8">
                    <div>
                        <h2 className="text-3xl font-bold text-gray-800"><span className='block text-sm'>Search location<MapPinned className='inline' /></span> {searchdata.name}</h2>
                        <p className="text-gray-600 mt-1">{searchdata.weather[0].description}</p>
                    </div>
                    <img
                        src={`https://openweathermap.org/img/wn/${searchdata.weather[0].icon}@2x.png`}
                        alt={searchdata.weather[0].description}
                        className="w-20 h-20"
                    />
                </div>
                <div className="text-5xl font-bold text-gray-800 mb-8">
                    {Math.round(searchdata.main.temp)}°C
                </div>
                <div className="grid grid-cols-2 gap-4">
                    <div className="flex items-center gap-2">
                        <Thermometer className="text-blue-500" />
                        <div>
                            <p className="text-sm text-gray-600">Feels like</p>
                            <p className="font-semibold">{Math.round(searchdata.main.feels_like)}°C</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
                        <Droplets className="text-blue-500" />
                        <div>
                            <p className="text-sm text-gray-600">Humidity</p>
                            <p className="font-semibold">{searchdata.main.humidity}%</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
                        <Wind className="text-blue-500" />
                        <div>
                            <p className="text-sm text-gray-600">Wind Speed</p>
                            <p className="font-semibold">{searchdata.wind.speed} m/s</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
                        <Cloud className="text-blue-500" />
                        <div>
                            <p className="text-sm text-gray-600">Pressure</p>
                            <p className="font-semibold">{searchdata.main.pressure} hPa</p>
                        </div>
                    </div>
                </div>
                
            </div>

           {hide?"":<Searchforecast />}
                   </>
    )
}

export default Searchcard